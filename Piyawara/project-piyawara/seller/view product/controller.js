...
// Author: Generated for Seller Dashboard
// Date: 2025
...
class SellerDashboard {
  ...
  setupEventListeners() {
    ...
    const decreaseBtn = document.querySelector(
      '.stock-btn[onclick="decreaseStock()"]'
    );
    const increaseBtn = document.querySelector(
      '.stock-btn[onclick="increaseStock()"]'
    );
  ...
  setupFormValidation() {
    ...
    const requiredFields = ["productName", "description"];
  ...
  handleMenuClick(e) {
    ...
    console.log(
      `Selected menu: ${
        clickedItem.querySelector("span:nth-child(2)").textContent
      }`
    );
  }
  ...
  getStepIndexFromSectionTitle(sectionTitle) {
    ...
    return sectionMapping[sectionTitle] !== undefined
      ? sectionMapping[sectionTitle]
      : -1;
  }
  ...
  isBasicInfoComplete(section) {
    ...
    const hasRequiredFields =
      productName?.value.trim() && category?.value && description?.value.trim();

    // Optional: Require at least one keyword and one image
    const hasOptionalContent =
      this.keywords.length > 0 && this.uploadedImages.length > 0;
  ...
  isProductSpecComplete(section) {
    ...
    const deliveryOptions = section.querySelectorAll(
    ...
    return brand?.value.trim() && deliveryOptions.length > 0 && unitType?.value;
  }
  ...
  isPriceStockComplete(section) {
    ...
    const paymentOptions = section.querySelectorAll(
  ...
  validateForm() {
    ...
    return (
      productName?.value.trim() && description?.value.trim() && category?.value
    );
  }
    ...
    const formData = {
      ...
      pricing: {
        ...
        paymentOptions: Array.from(
          document.querySelectorAll('input[name="pay"]:checked')
        ).map((cb) => cb.value),
      },
